...
function createTypesAsString(cmpMeta, _importPath) {
    ...
    const stencilComponentAttributesOptional = attributesToMultiLineString(Object.assign({}, propAttributes, eventAttributes), true);
    return {
        tagNameAsPascal: tagNameAsPascal,
        StencilComponents: `
interface ${tagNameAsPascal} {${stencilComponentAttributes !== '' ? `\n${stencilComponentAttributes}\n` : ''}}`,
        JSXElements: `
interface ${jsxInterfaceName} extends StencilHTMLAttributes {${stencilComponentAttributesOptional !== '' ? `\n${stencilComponentAttributesOptional}\n` : ''}}`,
        global: `
...
function generateServiceWorkers(config, compilerCtx, buildCtx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const promises = wwwServiceOutputs.map((outputTarget) => __awaiter(this, void 0, void 0, function* () {
            yield generateServiceWorker(buildCtx, outputTarget, workbox);
        }));
        yield Promise.all(promises);
    });
}
function generateServiceWorker(buildCtx, outputTarget, workbox) {
    return __awaiter(this, void 0, void 0, function* () {
...
function generateEsmLoader(config, compilerCtx, outputTarget) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield compilerCtx.fs.writeFile(pathJoin(config, loaderPath, 'package.json'), packageJsonContent);
        yield compilerCtx.fs.writeFile(pathJoin(config, loaderPath, 'index.d.ts'), indexDtsContent);
    });
}
...
function minifyStyle(config, compilerCtx, diagnostics, styleText, filePath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const cacheKey = compilerCtx.cache.createKey('minifyStyle', 'clean-css4.1.111', styleText, MINIFY_CSS_PROD);
        const cachedContent = yield compilerCtx.cache.get(cacheKey);
        if (cachedContent != null) {
...
function minifyCore(config, compilerCtx, sourceTarget, input) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let cacheKey;
        if (compilerCtx) {
            cacheKey = compilerCtx.cache.createKey('minifyCore', '180827145242', opts, input);
            const cachedContent = yield compilerCtx.cache.get(cacheKey);
            if (cachedContent != null) {
                return {
...
function autoprefixCssMain(config, compilerCtx, input, autoprefixConfig) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const cacheKey = compilerCtx.cache.createKey('autoprefix', 'autoprefixer8.6.5_postcss6.0.211', input, autoprefixConfig);
        const cachedContent = yield compilerCtx.cache.get(cacheKey);
        if (cachedContent != null) {
            // let's use the cached data we already figured out
            return cachedContent;
...
function inject(code, id, mod1, mod2, sourceMap, parse) {
    ...
    function handleReference(node, name, keypath, parent) {
        if ((mod1.has(keypath) || mod2.has(keypath)) && !scope.contains(name) && !imports[name]) {
            ...
            var moduleName = void 0, hash = void 0;
    ...
    walk(ast, {
        enter: function enter(node, parent) {
            ...
            if (isReference(node, parent)) {
                const _flatten = flatten(node), _name = _flatten.name, keypath = _flatten.keypath;
...
const GLOBAL_PATH = path.join(__dirname, '..', 'sys', 'node', 'rollup-node-globals-global.js');
const BROWSER_PATH = path.join(__dirname, '..', 'sys', 'node', 'rollup-node-globals-browser.js');
...
const firstpass = new RegExp('(?:' + mods.map(escape$1).join('|') + ')', 'g');
    ...
    return {
        ...
        resolveId: function resolveId(importee, importer) {
            ...
            if (importee === FILENAME) {
                const _id = crypto.randomBytes(15).toString('hex');
...
function deriveModule(config, compilerCtx, buildCtx, sourceTarget, isBrowser, moduleList) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Promise.all(module.list.map((chunk) => __awaiter(this, void 0, void 0, function* () { return deriveChunk(config, compilerCtx, buildCtx, sourceTarget, isBrowser, chunk); })));
        return module;
    });
}
function deriveChunk(config, compilerCtx, buildCtx, sourceTarget, isBrowser, chunk) {
    return __awaiter(this, void 0, void 0, function* () {
...
class Compiler {
    constructor(rawConfig) {
        ...
        if (this.isValid) {
            ...
            config.logger.debug(`compiler build: 180827145242`);
